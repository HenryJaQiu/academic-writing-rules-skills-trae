...
当用户已明确目标会议/期刊、需要切换 venue 或生成投稿平台字段时，调用 `venue-submission-adapter`：
...
`paper-intake-router -> academic-paper-factory -> literature-survey-builder -> paper-ratchet-optimizer -> citation-reality-guard -> reviewer-risk-audit`
