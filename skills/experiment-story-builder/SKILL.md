...
无论是哪种论文类型，只要实验承担主证据，就应额外检查：

- 他人是否能根据 caption、appendix、配置说明和训练/评估协议复现实验
- 数据过滤、预处理、seed、预算、early stopping、model selection 是否写清
- 关键结果是否区分“主文必复现”和“附录补充结果”
- 是否存在只保留最优 run、只保留最优 metric、或只展示成功 case 的风险

### `human-study guard`

若实验包含用户研究、人工评测、标注员判断、专家打分或问卷：

- 样本来源、招募方式、纳入/排除标准是否清楚
- task instruction、rating rubric、randomization / counterbalancing 是否说明
- IRB / ethics、consent、compensation、数据处理与隐私边界是否真实表述
- 统计方法是否匹配样本量与测量尺度，避免把弱主观信号写成强结论
...
若启用 `reproducibility guard`，还要额外查：

- 训练/评估脚本是否依赖未说明的内部默认参数
- 数据切分、过滤、去重或清洗是否缺失说明
- 是否没有说明如何从 checkpoint 选择最终报分

若启用 `human-study guard`，还要额外查：

- 评测标准是否模糊到不同 annotator 会给出完全不同判断
- 是否缺 inter-rater agreement、annotator training 或最基本的人群描述
- 是否把小样本主观偏好写成普适结论
...
若启用 `human-study guard`，额外要求：

- 区分 independent samples、paired design、within-subject 或 between-subject
- 说明样本量是否足以支撑当前统计检验；若不足，必须降级结论
- 主观量表、Likert、ranking、preference test 不要套用不合适的统计口径
...
若启用 `reproducibility guard`，caption 或邻近正文至少应能让读者知道：

- 结果来自哪个 split / seed protocol / checkpoint selection rule
- 误差线、阴影、箱线图或 violin plot 的具体统计含义

若启用 `human-study guard`，caption 或邻近正文至少应能让读者知道：

- 样本量、任务设置、评分尺度、标注主体或受试者来源
- 图中误差条代表的是 participant-level variance、annotator variance 还是 bootstrap CI
...
若启用 `reproducibility guard`：

- 主文至少保留让 reviewer 判断“结果是否可信”的最小复现信息
- 附录承接完整超参数、硬件、运行时、数据处理和额外 seeds 结果

若启用 `human-study guard`：

- 主文至少保留 participants / annotators、task design、主统计结论
- 附录承接 instruction、问卷、界面、rubric、更多分组分析和 ethics 细节
...
- “我无法根据正文和附录复现你的关键结果”
- “human study / annotation protocol 描述太弱，统计结论不可信”
...
- 是否涉及 human study、人工评测、标注或问卷
- 当前是否已有可复现所需的配置、数据处理和 checkpoint 选择说明
...
- 若需要，附 reproducibility 与 human-study 风险清单
