...
若以下任一项不明确，默认不要直接进入改单轮，而应先向用户确认：

- 本轮只允许优化哪一个目标
- 是否允许改动 `claim` 强度、标题、abstract / intro / conclusion
- 是否允许改动 `.bib`、引用表述、figures、appendix 或 checklist
- 是否允许重新编译、调版面、改图注或移动正文/附录内容
- 本轮是只做 wording 收紧，还是允许补证据/补实验计划
- 若对象是 rebuttal / response，哪些动作已经完成，哪些只是计划完成

默认询问优先级：

1. 本轮唯一目标
2. 是否允许动 `claim` 边界
3. 是否允许动 `.bib` / figures / appendix
4. 是否允许重编译或版面变动
5. 已完成 vs 计划完成
...
- 默认按最保守改单轮处理
- 默认不改 `claim` 边界之外的结构性内容
- 默认不碰 `.bib`、figures、appendix 与 checklist
- 默认不承诺新证据或新实验已完成
...
- [ ] 陈述他人工作是否优先使用一般过去时；陈述本文工作是否优先使用一般现在时或现在完成时
- [ ] 若本轮对象是 rebuttal / response，是否优先使用现在完成时描述已完成的澄清、补充和修正
- [ ] 是否把普通术语替换成了不自然的“高级词”，或混淆了近义词边界，如 `variety` / `variability`、`judgment` / `adjudication`
...
- 是否允许动 `claim` 强度、abstract / intro / conclusion 或 contribution bullets
- 若信息缺失，先向用户询问而不是乐观假设
...
- `这轮只允许我修哪一个问题？`
- `是否允许我收紧或改动 claim 强度？`
- `是否允许我改 .bib / figures / appendix / checklist？`
- `是否允许重新编译或触碰版面与图注？`
- `如果是 rebuttal / response，哪些改动已经完成，哪些只是计划完成？`
