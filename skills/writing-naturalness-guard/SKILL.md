...
它也吸收了 `Humanizer` 一类仓库里最有价值的部分，但保持成面向学术论文的版本：

- 用更明确的模式库，而不是只说“读起来像 AI”
- 允许可选的 voice calibration，但只在适合的 section 使用
- 加入“最终明显 AI 痕迹审计 + 第二次改写”，避免第一轮改写后仍残留机器节奏
...
6. 校正时态纪律与词汇纪律，避免“语法通顺但学术上别扭”的 AI 式改写
...
- `时态纪律`
  - 陈述他人已发表工作，优先用一般过去时
  - 陈述本文工作，优先用一般现在时或现在完成时
  - 写 `response to reviewer`、`author response`、`revision memo` 时，优先多用现在完成时
- `词汇纪律`
  - 优先使用语义直接、学术共同体自然使用的词
  - 若某个词更像华丽替换、法律/行政腔或生硬同义词，默认判为高风险风格信号
  - 对近义词必须按真实语义区分，例如 `variety` 不等于 `variability`，`judgment` 往往比 `adjudication` 更自然、更贴近普通学术写作

## 可选 voice calibration

若用户能提供 2-3 段自己满意的既有写作样本，可做有限的 voice calibration：

- 学习句长偏好、节奏、常用转承方式
- 学习是否偏简洁、偏保守、偏直接
- 只在 `Abstract / Introduction / Discussion / Rebuttal` 这类需要作者声音的 section 里参考

不应做的事：

- 不模仿口头禅到影响学术清晰度
- 不把方法、理论、实验节改成“有个人风格但不严谨”
- 不在没有样本时假装做了个性化匹配
...
- 过多使用 signposting，如 `Let's dive in`, `Here is what you need to know`
- 过多 persuasive framing，如 `At its core`, `what matters is`, `the value proposition is clear`
- diff-anchored 写法明显，例如只描述“这次改了什么”，不描述内容本身
- em/en dash 过多，形成模板化停顿节奏
- 时态混乱：讲他人工作、本文工作和 rebuttal 行动时来回切换，造成“像拼接文本”
- 用词看似高级但不自然，如把普通学术表达改成法律/行政腔、或把常见词替换成生硬近义词
...
若以下任一项不明确，默认不要直接开始自然化改写，而应先向用户确认：

- 是否只允许做风格自然化，不允许改内容
- 是否允许轻微收紧 wording，但不能改变 claim 强度
- 是否已经完成引用真实性核验
- 是否允许根据用户既有文本做 `voice calibration`
- 若允许 `voice calibration`，哪些样本才算可信写作样本
- 是否涉及投稿临近场景，需要额外保守处理 disclosure / integrity 风险

默认询问优先级：

1. 是否只改风格、不改内容
2. 是否已完成引用真实性核验
3. 是否允许 `voice calibration`
4. 是否存在投稿临近的 disclosure / integrity 风险

若用户没有明确回答：

- 默认只做风格层改写，不改技术内容和 claim 强度
- 默认不假设引用已核验
- 默认不启用 `voice calibration`
- 默认把可疑引用与合规问题继续视为优先级高于自然化
...
- significance inflation，如 `pivotal`, `transformative`, `groundbreaking`
- vague attribution，如 `experts believe`, `observers note`, `it is widely recognized`
- signposting announcements，如 `Let's dive in`, `Here's what you need to know`
- persuasive authority tropes，如 `At its core`, `the value proposition is clear`
- negative parallelisms，如 `it's not just X, it's Y`
- hyphenated word pair overuse，如 `data-driven`, `cross-functional`, `client-facing`
- diff-anchored writing，如只围绕“改了什么/新增什么”而不描述现在是什么
- 他人工作与本文工作时态混用，例如 related work 用现在时泛讲他人实验、本文贡献却退回过去时
- 用词疑似“华丽化替换”而非精确表达，例如 `adjudication`、`aforementioned` 一类不自然学术词
- 术语近义误用，例如把 `variety` 写成 `variability`，或把普通 `judgment` 写成不必要的 `adjudication`
...
- `Response / Rebuttal`：优先用现在完成时描述已完成澄清、已补充证据、已修正表述，避免攻击性和空泛承诺

同时显式校正：

- related work 中他人工作是否统一成过去时
- 本文贡献、图表说明和正文 claim 是否统一成现在时或现在完成时
- rebuttal / response 中是否使用了更稳妥的现在完成时，而不是把动作写得像未来空头承诺
- 词汇是否为了“显得高级”而偏离常见学术表达
...
- 能直接用句号、逗号、冒号的地方，不依赖 em/en dash 制造“有风格感”
- 删除不必要的 inline-header、碎片化小标题和列表腔

### Step 5.5: 可选 voice matching

若有可信写作样本，才做这一步：

- 优先匹配节奏和克制程度，而不是模仿表层口癖
- 只做轻量贴近，不把用户旧文的不良习惯复制过来
- 如果样本和当前稿件 genre 差异很大，宁可不用
...
- 这段还有没有“明显是 AI 写的”信号
- 还有没有 promotional language、vague attribution、机械排比
- 还有没有残留 signposting、模板结尾、聊天式 artifacts
- 还有没有靠同义词替换伪装自然化的问题
- 时态是不是仍然像从不同版本拼接出来的
- 术语和普通学术词是不是被替换成了不自然的“高级词”

若答案是“有”，再做一轮小范围二次改写，而不是整段重来。
...
- 过度使用 `At its core`, `value proposition`, `important to note`, `landscape`, `showcasing`
- 一串 `data-driven`, `cross-functional`, `human-centered` 这类连字符对
- 用 em/en dash 连续制造节奏感
- 先说“这段新增了/替换了/优化了什么”，而不是直接说内容是什么
...
- Related Work 里他人工作时态反复摇摆，像自动拼接
- rebuttal / response 里不用现在完成时，导致已做动作、将做动作和一般事实混在一起
- 把普通词硬改成 `adjudication` 这类不自然高级词，或混淆 `variety` / `variability`
...
- 是否允许使用用户既有文本做 `voice calibration`
- 若信息缺失，先向用户询问而不是默认放开改写边界

## 标准交付

- 若关键信息缺失，先给 `Clarifications Needed`
- 风格风险诊断
- 风险分级：`P0 / P1 / P2`
- 是否需要先做引用核验或 disclosure 风险处理
- 是否使用了 voice calibration
- 改写后的文本
- 保持不变的技术口径说明
- 仍建议人工复核的句子
- 若命中，附 `Tense Fixes / Lexical Precision Fixes`
...
- `这次是否只允许做风格自然化，不改内容？`
- `引用真实性是否已经核验过？`
- `是否允许我参考你既有写作样本做 voice calibration？`
- `当前是否临近投稿，需要把 disclosure / integrity 风险放在自然化前面？`
